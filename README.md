...
**Windows 用户**：直接下载最新版单文件 exe，双击即可运行，**无需安装 Python**。

👉 [点此下载 DesktopPet.exe](https://github.com/lyc047/desktop-pet/releases/latest)

想自己跑源码的话，看下面的[快速开始](#快速开始)。

## 🛡️ 下载/运行被拦截怎么办

exe **没有数字签名**（个人开源项目普遍如此），首次下载/运行被 SmartScreen、浏览器或杀毒软件提示属于**正常现象**，文件本身是安全的：

- **下载时浏览器提示「此文件可能损害你的设备」**：点旁边的「保留」→「仍然保留」
- **运行提示「Windows 已保护你的电脑」**：点「更多信息」→「仍要运行」
- **杀毒软件误报**（360/火绒/Defender 等）：点「允许运行」或在隔离区恢复，也可添加信任

**核对文件完整性**：每个 Release 页面的说明里都有该版本的 **SHA-256 校验值**。下载后执行：

```bash
certutil -hashfile DesktopPet.exe SHA256
```

结果与 Release 页面说明中的校验值一致，即确认文件完整可信。

> 如果实在不放心 exe，也可以走下面的[快速开始](#快速开始)用 Python 源码直接运行，效果完全一致。
